...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wandb-project", type=str, default="IGAE")
...
def main() -> None:
    ...
    from vae_rae.conditioning import VaeSigregConditioner, DinoCLSConditioner
    ...
    conditioner_type = str(vae_cond_cfg.get("type", "vaesigreg"))

    if conditioner_type == "dino_cls":
        conditioner = DinoCLSConditioner(
        ...
        vae_src_path = str(vae_cond_cfg.get("vae_src_path", str(project_root.parent / "stage1_IGAE" / "src")))
    ...
    if use_cached_latents:
        ...
        if hasattr(conditioner, "vae"):
            ...
            if cached_cls_dim != conditioner.cond_dim:
                raise ValueError(f"Cached cls_dim={cached_cls_dim} does not match conditioner cond_dim={conditioner.cond_dim}")
        print(
    ...
    ckpt_save_mode = str(ckpt_cfg.get("save_mode", "full")).lower()
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        # epoch 체크포인트 저장 (save_every_n_epochs=0 이면 비활성)
        should_save_ckpt = (ckpt_save_every > 0) and (
            (epoch % ckpt_save_every == 0) or (epoch == epochs)
        )
        if should_save_ckpt:
