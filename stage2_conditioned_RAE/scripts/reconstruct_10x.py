#!/usr/bin/env python3
"""
한 이미지에 대해 VAE-SIGREG-RAE 모델로 10번 reconstruction을 수행하고
원본 + 10개 결과를 그리드로 저장합니다.
"""
...
RAE_SRC  = THIS_DIR.parent / "vendor" / "rae_src"
VAE_RAE_SRC = THIS_DIR.parent / "src"
VAE_SAE_SRC = THIS_DIR.parent.parent / "stage1_IGAE" / "src"
...
# ── 메인 ───────────────────────────────────────────────────────────────────────
CKPT_PATH   = Path("/scratch/x3411a10/VAE-SIGREG/VAE-SIGREG-RAE/ckpts/1e3_200ep/vae_sigreg_1e3_200ep_20260502_104042/best.pt")
IMAGE_PATH  = Path("/scratch/x3411a10/datasets/ffhq256/imagefolder/val/images/00025.png")
OUTPUT_PATH = THIS_DIR.parent / "visualizations" / "reconstruct_10x_result.png"
N_RECON     = 10
IMAGE_SIZE  = 256
...
# ── Stage-2: DiTwDDTHeadVAECond (EMA weights) ─────────────────────────────────
...
img_cropped = crop_tf(img)
img_tensor  = to_tensor(img_cropped).unsqueeze(0).to(DEVICE)  # (1, 3, H, W)
print(f"input image shape: {img_tensor.shape}")

# ── 10번 reconstruction ────────────────────────────────────────────────────────
results = [img_tensor.cpu()]  # 첫 번째: 원본

with torch.no_grad():
    for i in range(N_RECON):
        # conditioner는 sample mode → 매번 다른 z 샘플링
        cond = conditioner(img_tensor)          # (1, cond_dim)
        z0   = torch.randn(1, *latent_size, device=DEVICE)
        zhat = sample_fn(z0, model.forward, cond=cond)[-1]
        recon = rae.decode(zhat).clamp(0, 1)    # (1, 3, H, W)
        results.append(recon.cpu())
        print(f"  recon {i+1:02d}/{N_RECON} done")

# ── 그리드 저장 ─────────────────────────────────────────────────────────────────
# [원본, recon1, ..., recon10] → 1행 11열 또는 2행으로 자동
grid = make_grid(torch.cat(results, dim=0), nrow=11, padding=4, normalize=False)
save_image(grid, OUTPUT_PATH, normalize=False)
print(f"\n결과 저장: {OUTPUT_PATH}")
print(f"총 이미지 수: 1 (원본) + {N_RECON} (재구성) = {1 + N_RECON}")
