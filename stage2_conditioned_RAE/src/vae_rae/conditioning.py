...
class DinoCLSConditioner(nn.Module):
    """DINO CLS token directly as condition vector (baseline, no IGAE).

    cond_dim = 768 (DINOv2-base CLS dimension).
    """
    ...
    def sample_prior(self, batch_size: int, device: torch.device | str, dtype: torch.dtype | None = None) -> torch.Tensor:
        ...
        return torch.randn(batch_size, self._cond_dim, device=device, dtype=dtype)
