#!/bin/bash
# Overcomplete VAE with precision-signal consistency:
#   rec + beta_kl * KL
...
RUN_TAG="s1_grid_b1e-4_precsig_l0p05_k2"
...
PYTHONUNBUFFERED=1 python train_run.py \
  ...
  --wandb_tags    "precision_signal" "beta1e-4" "lambda0p05" "kappa2" "overcomplete" "no_sigreg" \
  2>&1 | tee -a "$LOG"
