#!/bin/bash
# Reconstruction + beta KL + stronger coupled latent sparsity:
#   rec + beta_kl * KL + 0.1 * mean(|mu| * sigma)
...
RUN_TAG="s1_grid_b1e-4_kl_musigl1_l0p1"
...
PYTHONUNBUFFERED=1 python train_run.py \
  ...
  --wandb_tags    "kl" "mu_sigma_l1" "beta1e-4" "lambda0p1" "no_sigreg" \
  2>&1 | tee -a "$LOG"
