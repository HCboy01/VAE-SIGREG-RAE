#!/bin/bash
# Overcomplete VAE with weaker precision-signal consistency:
#   rec + beta_kl * KL
#     + lambda * mean(relu((1/sigma^2 - 1) - kappa * mu^2))
...
RUN_TAG="s1_grid_b1e-4_precsig_l0p02_k2"
...
PYTHONUNBUFFERED=1 python train_run.py \
  ...
  --lambda_precision_signal 0.02 \
  --precision_signal_kappa 2 \
  --num_projections 512 \
  ...
  --wandb_tags    "precision_signal" "beta1e-4" "lambda0p02" "kappa2" "overcomplete" "no_sigreg" \
  2>&1 | tee -a "$LOG"
