#!/bin/bash
# Weaker batch feature-axis sigma deviation penalty at beta=1e-4, lambda_sigreg=100.
#
# Run:
#   s1_grid_b1e-4_l100_sigmadev_l1_sd0p05_below
...
METRIC="l1"
RUN_TAG="s1_grid_b1e-4_l100_sigmadev_${METRIC}_sd0p05_below"
...
PYTHONUNBUFFERED=1 python train_run.py \
  ...
  --wandb_tags    "sigmadev" "$METRIC" "only_below" "sd0p05" "beta${BETA}" "lambda${LAMBDA_SIGREG}" \
  2>&1 | tee -a "$LOG"
