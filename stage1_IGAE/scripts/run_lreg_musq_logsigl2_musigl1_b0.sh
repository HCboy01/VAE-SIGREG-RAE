#!/bin/bash
# Reconstruction + compact latent regularizer:
#   mean(mu^2) + mean(logsigma^2) + mean(|mu * sigma|)
...
RUN_TAG="s1_grid_b0_lreg_musq1_logsigl2a1_musigl1l1"
...
PYTHONUNBUFFERED=1 python train_run.py \
  ...
  --lambda_mu_sq 1 \
  --lambda_logsigma_l2 1 \
  --lambda_mu_sigma_l1 1 \
  --num_projections 512 \
  ...
  --wandb_tags    "lreg" "mu_sq" "logsigma_l2" "mu_sigma_l1" "beta0" "no_sigreg" \
  2>&1 | tee -a "$LOG"
