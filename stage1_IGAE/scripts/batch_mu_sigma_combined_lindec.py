"""
Combined mu-sigma scatter batch script for all lindec checkpoints.
...
Output: visualizations/mu_sigma_combined_lindec/<run_name>.png
"""
...
N_IMAGES    = 3000
IMG0_IDX    = 0     # index into x_sample for per-dim scatter (Row 2)
SIGMA_THRS  = [0.8, 0.5, 0.2]  # annotation thresholds for Row 2
# ────────────────────────────────────────────────────────────────────────────
...
def make_combined(run_name, mu, logvar, beta, lam):
    ...
    # ── Row 1: 5 quantile dims ───────────────────────────────────────────
    Q_LABELS = ["Min KL (0%)", "25%", "Median (50%)", "75%", "Max KL (100%)"]
    Q_COLORS = ["#4da6e8", "#6bbf72", "#e8c44d", "#e87c4d", "#c94545"]
    q_indices = [int(q * (D - 1)) for q in [0.0, 0.25, 0.50, 0.75, 1.0]]
    dims_q    = [kl_dim.argsort()[i].item() for i in q_indices]

    # ── Row 2: per-dim scatter for image IMG0_IDX ────────────────────────
    mu0    = mu[IMG0_IDX].numpy()      # [D]
    sig0   = sigma[IMG0_IDX].numpy()   # [D]
    # per-dim KL for this single image (for colormap)
    kl0    = 0.5 * (mu[IMG0_IDX].pow(2) + logvar[IMG0_IDX].exp()
                    - logvar[IMG0_IDX] - 1).numpy()  # [D]

    # ── Figure ───────────────────────────────────────────────────────────
    fig = plt.figure(figsize=(26, 10))
    fig.suptitle(
        f"{run_name}   beta={beta}  lambda={lam}\n"
        f"kl range (avg): [{kl_dim.min():.5f}, {kl_dim.max():.5f}]  "
        f"N={N_IMAGES}  |  prior: mu=0 (dashed), sigma=1 (dotted)",
        fontsize=10,
    )

    gs = gridspec.GridSpec(2, 5, figure=fig, hspace=0.45, wspace=0.32,
                           top=0.88, bottom=0.07, left=0.05, right=0.98)

    # ── Row 1 panels ─────────────────────────────────────────────────────
    for col, (dim, lbl, col_c) in enumerate(zip(dims_q, Q_LABELS, Q_COLORS)):
        ax = fig.add_subplot(gs[0, col])
        mu_d    = mu[:, dim].numpy()
        sigma_d = sigma[:, dim].numpy()
        kl_val  = kl_dim[dim].item()
        ax.scatter(mu_d, sigma_d, s=5, alpha=0.30, color=col_c, linewidths=0)
    ...
    # ── Row 2: per-dim scatter for image 0 (span all 5 cols) ─────────────
    ax2 = fig.add_subplot(gs[1, :])

    norm_kl = mcolors.LogNorm(vmin=max(kl0.min(), 1e-6), vmax=kl0.max() + 1e-9)
    sc = ax2.scatter(mu0, sig0, c=kl0, cmap="plasma", norm=norm_kl,
    ...
    # sigma threshold annotations
    thr_lines = [("#2196F3", 0.8), ("#4CAF50", 0.5), ("#F44336", 0.2)]
    for color, thr in thr_lines:
        n = (sig0 < thr).sum()
        ax2.axhline(thr, color=color, lw=0.8, ls="--", alpha=0.7,
                    label=f"sigma<{thr}: {n} dims")

    ax2.set_title(
        f"Per-dim mu vs sigma  —  image idx {IMG0_IDX}  (each point = 1 latent dim, D={D})\n"
    ...
    fig.text(0.005, 0.72, "KL quantile\n(0/25/50/75/100%)",
             fontsize=8, va="center", rotation=90, color="#555")
