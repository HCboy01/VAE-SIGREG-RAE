#!/bin/bash
# Overcomplete VAE with abs-mu precision-signal consistency:
#   rec + beta_kl * KL
#     + lambda * mean(relu((1/sigma^2 - 1) - kappa * |mu|))
...
RUN_TAG="s1_grid_b1e-4_precsig_abs_l0p05_k0p25"
...
PYTHONUNBUFFERED=1 python train_run.py \
  ...
  --lambda_precision_signal 0.05 \
  --precision_signal_kappa 0.25 \
  --precision_signal_target abs \
  --num_projections 512 \
  ...
  --wandb_tags    "precision_signal_abs" "beta1e-4" "lambda0p05" "kappa0p25" "overcomplete" "no_sigreg" \
  2>&1 | tee -a "$LOG"
