#!/bin/bash
# Compact confidence sparsity:
#   sparse mu, and penalize low sigma only where |mu| is inactive.
...
RUN_TAG="s1_grid_b1e-4_mu_l1_inconf_m0p05_c0p1_t0p1_tau0p05"
...
PYTHONUNBUFFERED=1 python train_run.py \
  ...
  --wandb_tags    "mu_l1" "inactive_conf" "beta1e-4" "mu0p05" "conf0p1" "no_sigreg" \
  2>&1 | tee -a "$LOG"
