#!/bin/bash
# Compare batch feature-axis sigma deviation penalties at beta=1e-4, lambda_sigreg=100.
#
# Runs:
...
BETA="1e-4"
LAMBDA_SIGREG="100"
LAMBDA_SIGMA_DEV="0.1"

for METRIC in l1 l2; do
  RUN_TAG="s1_grid_b1e-4_l100_sigmadev_${METRIC}_sd0p1_below"
  ...
  PYTHONUNBUFFERED=1 python train_run.py \
    ...
    --wandb_tags    "sigmadev" "$METRIC" "only_below" "sd0p1" "beta${BETA}" "lambda${LAMBDA_SIGREG}" \
    2>&1 | tee -a "$LOG"
