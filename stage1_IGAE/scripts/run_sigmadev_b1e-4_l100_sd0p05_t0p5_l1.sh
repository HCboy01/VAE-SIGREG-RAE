#!/bin/bash
# Weaker one-sided sigma deviation penalty with target sigma=0.5.
#
# Run:
...
BETA="1e-4"
LAMBDA_SIGREG="100"
LAMBDA_SIGMA_DEV="0.05"
SIGMA_DEV_TARGET="0.5"
METRIC="l1"
RUN_TAG="s1_grid_b1e-4_l100_sigmadev_${METRIC}_sd0p05_t0p5_below"
...
PYTHONUNBUFFERED=1 python train_run.py \
  ...
  --sigma_dev_target "$SIGMA_DEV_TARGET" \
  ...
  --wandb_tags    "sigmadev" "$METRIC" "only_below" "sd0p05" "target0p5" "beta${BETA}" "lambda${LAMBDA_SIGREG}" \
  2>&1 | tee -a "$LOG"
